...
for i in idx:
    ...
    dir_files_2.append('./results/' + dataset + '/supp_nomix_wnr' + i)
    dir_files_3.append('./results/' + dataset + '/model_wn' + i)
    dir_files_4.append('./results/' + dataset + '/supp_model100_wnr' + i)
...
ax.plot(epochs, mean_and_sem(accuracies_2, color='blueviolet', axis=0), color ='blueviolet', label='w/o memory mix')
...
#if not occlusions:
#    fig.savefig('./results/'+dataset+'_supp_linear.pdf')
#else:
#    fig.savefig('./results/' + dataset + '_supp_linear_occ.pdf')
